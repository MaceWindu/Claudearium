...
**`setup` fails at bootstrap with `Temporary failure resolving deb.debian.org` while a host VPN is connected** — same root cause as below (the host VPN breaks the WSL2 NAT DHCP, so the freshly-imported distro has no egress for `apt`). `setup` now runs net-repair inline before bootstrap to handle this automatically, so it should provision regardless; if it still fails, the host VPN is breaking the NAT vSwitch entirely (not just DHCP) — disconnect/split-tunnel the VPN for the one-time `setup`, then re-enable it and use `network repair` (below) for ongoing sessions.

**No internet in the distro while a host VPN is connected** — a host-side VPN (ProtonVPN and other WireGuard/TUN clients especially) can break the WSL2 NAT DHCP, leaving the distro's `eth0` with no IP and no default route. Run `.\claudearium.ps1 network repair` (assigns a static IP + default route, runs immediately), or set `"network": { "enabled": true }` in the profile and `reconcile` to make it automatic on every boot. Confirm with `.\claudearium.ps1 network status`. On Windows 11 22H2+, the cleaner fix is `networkingMode=mirrored` in `%USERPROFILE%\.wslconfig`. If `network repair` restores DNS/route but large transfers still hang, it's a tunnel-MTU mismatch — add `"mtu": 1280` to the `network` block. As a last resort on stubborn corporate VPNs, run an elevated PowerShell and raise the VPN adapter's interface metric above `vEthernet (WSL)`'s (`Get-NetIPInterface -InterfaceAlias '<VPN adapter>' | Set-NetIPInterface -InterfaceMetric 6000`) so WSL's NAT traffic prefers the physical route. See [wsl2-gotchas #26](./wsl2-gotchas.md#26-eth0-gets-no-dhcp-lease-no-ipv4-no-default-route-when-a-host-vpn-is-up-on-win10).

**Still no egress after `network repair` — `eth0` has an address + default route but nothing reaches the internet** — this is a *different* failure from the no-lease case above: a full-tunnel host VPN with a **kill switch** (ProtonVPN) that drops WSL's NAT'd packets at the Windows firewall layer. `network repair` (a layer-3 fix) can't help, and "Allow LAN connections" / IP split tunneling usually don't either. Use `vpnkit`, which tunnels WSL egress through a host process the kill switch permits: `.\claudearium.ps1 vpnkit install` then `.\claudearium.ps1 vpnkit start` (run both *before* `setup` to unblock bootstrap `apt`). Confirm the tunnel with `.\claudearium.ps1 vpnkit status` or `Get-Process wsl-gvproxy`. First launch may raise a Windows SmartScreen prompt on the unsigned `wsl-gvproxy.exe` — approve it and retry. See [design-decisions #33](./design-decisions.md#33-vpnkit--a-host-side-userspace-tunnel-for-kill-switch-host-vpns).
...
**A project's tab icon / background image isn't showing in Windows Terminal** — these are Windows Terminal *profile* settings delivered via a generated fragment, and WT reads fragments **only at startup**. After a `reconcile` / `project add` / `wt-profiles apply` that changes them, fully restart Windows Terminal (close all windows). Confirm the fragment exists with `.\claudearium.ps1 wt-profiles` (lists the generated profiles + the on-disk path). Note `tabColor` is unaffected — it's a `wt.exe` launch flag, not a profile setting, so it applies immediately. `backgroundImageOpacity` also has no visible effect unless the project sets a `backgroundImage`.
