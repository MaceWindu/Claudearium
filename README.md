...
- **Optional:** keep the distro working when a **host-side VPN** (e.g. ProtonVPN) breaks the WSL2 NAT DHCP — the `network` verb installs a boot-time `eth0` repair that restores connectivity (and the distro then egresses through the host VPN automatically). A no-op when the VPN is off. When even that isn't enough (a kill-switch VPN that drops egress despite a valid route), the `vpnkit` verb tunnels WSL traffic through a host userspace stack ([wsl-vpnkit](https://github.com/sakai135/wsl-vpnkit)).
