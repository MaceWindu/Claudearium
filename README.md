...
- **Optional:** keep the distro working when a **host-side VPN** (e.g. ProtonVPN) breaks the WSL2 NAT DHCP — the `network` verb installs a boot-time `eth0` repair that restores connectivity (and the distro then egresses through the host VPN automatically). A no-op when the VPN is off.
