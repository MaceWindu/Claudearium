...
AfterAll {
    Invoke-Claudearium -DistroName $script:distro -ProfilePath $script:profilePath `
        -Args @{ Verb='mount'; SubVerb='remove'; Arg=$script:guestPath; Force=$true } -AllowFail | Out-Null
...
Describe 'mount add' -Tag 'distro' {
    It 'writes the fstab entry between managed-block markers' {
        Invoke-Claudearium -DistroName $script:distro -ProfilePath $script:profilePath `
            -Args @{ Verb='mount'; SubVerb='add'; Arg=$script:hostPath; Guest=$script:guestPath; Mode='ro' }
...
Describe 'mount remove' -Tag 'distro' {
    It 'drops the fstab entry and unmounts' {
        Invoke-Claudearium -DistroName $script:distro -ProfilePath $script:profilePath `
            -Args @{ Verb='mount'; SubVerb='remove'; Arg=$script:guestPath; Force=$true }
